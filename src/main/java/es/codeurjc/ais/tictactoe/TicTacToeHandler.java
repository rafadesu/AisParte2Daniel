...
public class TicTacToeHandler extends TextWebSocketHandler {
	...
	//Añadimos un objeto StatisticsService
	private StatisticsService estadisticas = new StatisticsService();
			...
			case MARK:
				...
				if(empatado) {
					...
					for(Player jugador: Jugadores) {						
						//Intenta añadir el jugador, si el jugador ya esta en la lista, actualiza sus empates.
						estadisticas.anadirJugador(jugador);
						estadisticas.actualizarEmpatesJugador(jugador);
					...
					//comprobamos quien ha ganado para incrementar estadisticas tanto de los que han perdido como los que han ganado
					...
					if(wr.win){
						...
						for(Player jugador: Jugadores) {
							//Ganador
							if(jugador.getId()==msg.data.playerId) {
								estadisticas.anadirJugador(jugador);
								estadisticas.actualizarGanadasJugador(jugador);
								
							}else {//Perdedor
								estadisticas.anadirJugador(jugador);
								estadisticas.actualizarPerdidasJugador(jugador);
							}												
						}
						System.out.println(estadisticas);
					}					
